...
void box(int lin1, int col1, int lin2, int col2) { // CRIA��O DA CAIXA
    int i, j;
    linhaCol(lin1, col1);
    printf("+");
...
int selecionarUsuario() { // Fun��o para exibir e escolher um usu�rio pela lista
    int selecionado = 0;
    int tecla;
    ...
    while (1) {
        ...
        tecla = getch(); // Navega��o com as setas
        if (tecla == 224) {
            tecla = getch();
            if (tecla == 72) {  // Seta para cima
                if (selecionado <= 0) {
                    selecionado = total_usuarios - 1; // Se estiver no primeiro usu�rio, vai para o �ltimo
                } else {
                    selecionado--;
                }
            } else if (tecla == 80) {  // Seta para baixo
                if (selecionado >= total_usuarios - 1) { // Se estiver no primeiro usu�rio, vai para o primeiro
                    selecionado = 0;
                } else {
                    selecionado++;
                }
